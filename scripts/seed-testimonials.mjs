/**
 * Seed default recovery testimonials into MongoDB (idempotent upsert by slug).
 *
 * Usage:
...
try {
  ...
  for (let order = 0; order < testimonials.length; order++) {
    const t = testimonials[order];
    const r = await db.collection("testimonials").updateOne(
      { slug: t.slug },
      {
        $set: {
          slug: t.slug,
  ...
  console.log(`Seeded testimonials: ${upserted} inserted, ${updated} updated (${total} total in collection).`);
  console.log("Published stories: annie-g, kerrie-mchugh, daniel-r, marisol-t");
} finally {
  await client.close();
}
